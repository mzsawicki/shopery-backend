...
class S3Gateway:
    ...
    def upload_file(self, bucket: str, file_key: str, file: typing.BinaryIO) -> UploadResult:
        try:
            self._client.upload_fileobj(file, bucket, file_key)
            return UploadResult(success=True, uploaded_file_path=f"{self._url}/{bucket}/{file_key}")
